...
struct UnsupportedNoJumpIterativeKernel <:
       PermutationalInvariantDynamics.AbstractStaticPIKernel end

function no_jump_generator_matrix(plan)
    n=length(plan.basis)
...
@testset "No-jump-resolvent iterative methods" begin
    ...
    for backend in (:schur,:eigen)
        ...
        S=no_jump_generator_matrix(plan.no_jump)
    ...
    plan=NoJumpIterativePlan(model;memory_budget=Inf)
    ...
    work=NoJumpIterativeWorkspace(plan;krylovdim=18,recycle_dim=4,
                         memory_budget=Inf)
    S=no_jump_generator_matrix(plan.no_jump)
    ...
    compiled_plan=NoJumpIterativePlan(compiled;memory_budget=Inf)
    compiled_no_jump=NoJumpResolventPlan(compiled;memory_budget=Inf)
    @test no_jump_generator_matrix(compiled_plan.no_jump)≈S atol=2e-13 rtol=2e-13
    @test no_jump_generator_matrix(compiled_no_jump)≈S atol=2e-13 rtol=2e-13
    ...
    specialized_plan=NoJumpIterativePlan(specialized;memory_budget=Inf)
    independently_prepared=NoJumpIterativePlan(
    ...
    @test no_jump_generator_matrix(specialized_plan.no_jump)≈
        no_jump_generator_matrix(independently_prepared.no_jump) atol=2e-13 rtol=2e-13
    specialized_state=no_jump_iterative_steady_state(
    ...
    specialized_wrapped=no_jump_iterative_steady_state(specialized;method=:gmres,
        krylovdim=18,recycle_dim=4,atol=1e-10,rtol=1e-8,
        memory_budget=Inf)
    @test specialized_wrapped.data≈specialized_state.data atol=4e-10 rtol=4e-9
    specialized_work=NoJumpIterativeWorkspace(specialized_plan;krylovdim=8,
    ...
    PermutationalInvariantDynamics._apply_no_jump_iterative_gain!(specialized_gain,
    ...
    PermutationalInvariantDynamics._apply_no_jump_iterative_gain!(
    ...
    unfused=NoJumpIterativePlan(LiouvillianPlan(model;fuse_static=false);
        memory_budget=Inf)
    unfused_work=NoJumpIterativeWorkspace(unfused;krylovdim=8,recycle_dim=2,
        memory_budget=Inf)
    ...
    PermutationalInvariantDynamics._apply_no_jump_iterative_gain!(unfused_gain,
    ...
    correlated=NoJumpIterativePlan(
        LiouvillianPlan(correlated_model;fuse_static=false);
        memory_budget=Inf)
    correlated_work=NoJumpIterativeWorkspace(correlated;krylovdim=8,recycle_dim=2,
        memory_budget=Inf)
    ...
    PermutationalInvariantDynamics._apply_no_jump_iterative_gain!(correlated_gain,
    ...
    gmres=no_jump_iterative_steady_state(plan;method=:gmres,workspace=work,
        return_info=true,atol=1e-10,rtol=1e-8,memory_budget=Inf)
    @test gmres.converged
    @test gmres.method===:no_jump_iterative_gmres
    ...
    compiled_state=no_jump_iterative_steady_state(compiled;method=:gmres,
        krylovdim=18,recycle_dim=4,atol=1e-10,rtol=1e-8,
        memory_budget=Inf)
    @test compiled_state.data≈gmres.state.data atol=3e-10 rtol=3e-9

    fixed=no_jump_iterative_steady_state(plan;method=:fixed_point,krylovdim=18,
        maxrestarts=10,return_info=true,atol=1e-9,rtol=1e-7,
        memory_budget=Inf,rng=MersenneTwister(11))
    @test fixed.converged
    @test fixed.method===:no_jump_iterative_fixed_point
    ...
    driven_fixed=no_jump_iterative_steady_state(driven_qubit_model;
    ...
    larger_model=PIModel(larger_basis,(
        LocalJump(spin.jm;rate=0.7),
        LocalJump(spin.jp;rate=0.2),))
    larger_plan=NoJumpIterativePlan(larger_model;memory_budget=Inf)
    larger=no_jump_iterative_steady_state(larger_plan;method=:gmres,
    ...
    result=no_jump_iterative_resolvent(plan,rhs;shift,deflation,
    ...
    @test_throws ArgumentError no_jump_iterative_resolvent(plan,rhs;shift,deflation,
        workspace=work,memory_budget=resolvent_budget-1)
    bounded_resolvent=no_jump_iterative_resolvent(plan,rhs;shift,deflation,
    ...
    stationary_budget=work.accounted_peak_bytes+
        PermutationalInvariantDynamics._no_jump_iterative_stationary_output_bytes(plan)
    @test_throws ArgumentError no_jump_iterative_steady_state(plan;method=:gmres,
        workspace=work,memory_budget=stationary_budget-1)
    bounded_stationary=no_jump_iterative_steady_state(plan;method=:gmres,
    ...
    spectrum=no_jump_iterative_liouvillian_spectrum(plan;nev=2,krylovdim=18,
    ...
    @test spectrum.method===:no_jump_iterative_shift_invert
    ...
    @test_throws ArgumentError no_jump_iterative_liouvillian_spectrum(plan;
    ...
    step=no_jump_iterative_implicit_euler_step!(destination,plan,rho,dt,work;
    ...
    evolution=no_jump_iterative_implicit_euler(plan,rho,[0.0,dt,2dt];
    ...
    @test_throws ArgumentError no_jump_iterative_implicit_euler(plan,rho,[0.0,dt];
        workspace=work,memory_budget=1)
    @test_throws ArgumentError no_jump_iterative_implicit_euler(
    ...
    dark=NoJumpIterativePlan(dark_model;memory_budget=Inf)
    ...
    dark_S=no_jump_generator_matrix(dark.no_jump)
    ...
    @test_throws ArgumentError no_jump_iterative_steady_state(
    ...
    robust=NoJumpIterativePlan(defective_model;backend=:schur,memory_budget=Inf)
    ...
    robust_S=no_jump_generator_matrix(robust.no_jump)
    @test norm(-robust_S*robust_result-robust_source)<3e-11
    @test_throws ArgumentError NoJumpIterativePlan(defective_model;
    ...
    plan32=NoJumpIterativePlan(model32;memory_budget=Inf)
    ...
    @test_throws ArgumentError no_jump_iterative_steady_state(plan32;
        method=:fixed_point,krylovdim=4,maxrestarts=1,
        atol=nextfloat(0.0),rtol=Float32(1e-4),memory_budget=Inf)
    @test_throws ArgumentError NoJumpIterativePlan(model32;stability_atol=Inf,
        memory_budget=Inf)
    ...
    for (bits,mode) in ((64,RoundDown),(256,RoundUp))
        setrounding(BigFloat,mode) do
            setprecision(BigFloat,bits) do
                ...
                @test_throws ArgumentError NoJumpIterativePlan(
    ...
    @test_throws ArgumentError no_jump_iterative_implicit_euler(
        ...
        memory_budget=Inf)
    work32=NoJumpIterativeWorkspace(plan32;krylovdim=4,recycle_dim=1,
        memory_budget=Inf)
    fill!(work32.rhs,ComplexF32(3,1))
    saved_rhs32=copy(work32.rhs)
    @test_throws ArgumentError no_jump_iterative_implicit_euler_step!(
    ...
    @test_throws ArgumentError NoJumpIterativePlan(model;memory_budget=1)
    @test_throws ArgumentError NoJumpIterativeWorkspace(plan;memory_budget=1)
    @test_throws ArgumentError NoJumpIterativeWorkspace(plan;krylovdim=true,
        memory_budget=Inf)
    @test_throws ArgumentError NoJumpIterativeWorkspace(plan;recycle_dim=false,
        memory_budget=Inf)
    unsupported_plan=LiouvillianPlan(basis,(UnsupportedNoJumpIterativeKernel(),),
    ...
    other=NoJumpIterativePlan(PIModel(PIBasis(1,2),(
        LocalJump(spin.jm;rate=0.5),LocalJump(spin.jp;rate=0.1)));
        memory_budget=Inf)
    other_work=NoJumpIterativeWorkspace(other;memory_budget=Inf)
    @test_throws ArgumentError no_jump_iterative_steady_state(
    ...
    @test_throws ArgumentError NoJumpIterativePlan(PIModel(basis,(
        LocalJump(spin.jm;rate=-0.1),));memory_budget=Inf)
    # Bypassing a term constructor's Hermiticity check must not let a
    # non-GKSL Hamiltonian acquire the no-jump-resolvent iterative CPTP/contraction guarantees.
    ...
    cancelling_model=PIModel(basis,(
        ...
        LocalJump(spin.jm;rate=0.7),LocalJump(spin.jp;rate=0.2)))
    cancelling_plan=NoJumpIterativePlan(
    ...
    instant_reference_model=PIModel(basis,(
        LocalHamiltonian(spin.jx;rate=0.31),
        LocalJump(spin.jm;rate=0.7),LocalJump(spin.jp;rate=0.2)))
    instant_reference=NoJumpIterativePlan(
        instant_reference_model;memory_budget=Inf)
    instant=NoJumpIterativePlan(driven;time=instant_time,
    ...
    @test no_jump_generator_matrix(instant.no_jump)≈
        no_jump_generator_matrix(instant_reference.no_jump) atol=2e-13 rtol=2e-13
    instant_no_jump=NoJumpResolventPlan(driven;time=instant_time,
        parameters=instant_parameters,memory_budget=Inf)
    @test instant_no_jump.metadata.instantaneous
    @test no_jump_generator_matrix(instant_no_jump)≈
        no_jump_generator_matrix(instant_reference.no_jump) atol=2e-13 rtol=2e-13
    instant_state=no_jump_iterative_steady_state(driven;time=instant_time,
    ...
    @test_throws ArgumentError NoJumpIterativePlan(driven;memory_budget=Inf)
    @test_throws ArgumentError NoJumpIterativePlan(driven;
        parameters=instant_parameters,memory_budget=Inf)
    @test_throws ArgumentError NoJumpIterativePlan(driven;time=Inf,
        parameters=instant_parameters,memory_budget=Inf)
    @test_throws ArgumentError NoJumpIterativePlan(model;time=instant_time,
        memory_budget=Inf)
    @test_throws ArgumentError NoJumpResolventPlan(model;
        parameters=instant_parameters,memory_budget=Inf)
    @test_throws ArgumentError no_jump_iterative_steady_state(specialized;
    ...
    dephasing=NoJumpIterativePlan(PIModel(PIBasis(1,2),(
        LocalJump(spin.jz;rate=1.0),));memory_budget=Inf)
    dephasing_modes=no_jump_iterative_liouvillian_spectrum(dephasing;nev=1,shift=0.05,
    ...
    invariant_restriction=NoJumpIterativePlan(PIModel(symmetric_basis,(
    ...
    @test_throws ArgumentError NoJumpIterativePlan(PIModel(symmetric_basis,(
    ...
    PermutationalInvariantDynamics._apply_no_jump_iterative_gain!(pbody_gain,pbody,
