...
measure("prepared solver preflight",()->recommend_solver(
    prepared;task=:dynamics,algorithm=:rk4))
support_probe=ones(ComplexF64,129,129)
measure("dense exact support count",()->
    PermutationalInvariantDynamics._performance_matrix_nonzeros(support_probe))
...
measure("symmetric state validation",()->validate_state(entropy_state))
entropy_value=measure("symmetric validated entropy",()->von_neumann_entropy(entropy_state))
isapprox(entropy_value,log2(129);atol=2e-11)||error(
    "precision guard failed: symmetric entropy differs")
