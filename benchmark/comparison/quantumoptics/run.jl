...
function collective_setup(N)
    basis = SpinBasis(N // 2)
    lowering = sigmam(basis)
    state = dm(spinup(basis))
    generator_object = liouvillian(zero(GAMMA) * lowering, [lowering];
                                   rates=[GAMMA])
...
function local_setup(N)
    basis = SpinBasis(1 // 2)
    lowering = sigmam(basis)
    identity = identityoperator(basis)
    jumps = [tensor(ntuple(site -> site == particle ? lowering : identity, N)...)
             for particle in 1:N]
    excited = tensor(ntuple(_ -> spinup(basis), N)...)
    state = dm(excited)
    generator_object = liouvillian(zero(GAMMA) * first(jumps), jumps;
                                   rates=fill(GAMMA, N))
    generator = as_sparse_matrix(generator_object.data)
    input = collect(vec(state.data))
    output = similar(input)
...
function main(arguments=ARGS)
    ...
    package_version = string(Base.pkgversion(QuantumOptics))
    rows = NamedTuple[]
    for N in COLLECTIVE_SIZES
        push!(rows, benchmark_case(
            ...
            notes="SpinBasis(N/2), a single fixed-spin irrep",
