...
| No-jump resolvents and well-conditioned iterative open-system solvers | Independent implementation of published mathematics, specialized to PI Schur coordinates | G. Beugnot, P. Gregory, R. Robin, and A. Tilloy, *Well-conditioned iterative methods for large open quantum systems*, [arXiv:2608.30860v1 (2026)](https://arxiv.org/abs/2608.30860); `src/no_jump_iterative.jl`, `test/test_no_jump_iterative.jl`, and `docs/src/no_jump_iterative_solvers.md`. The implementation was written independently for the package's prepared-kernel, precision, ownership, and memory contracts; no third-party source expression or paper figure was copied |
| Complex-shift trace-deflated inexact IRAM and non-normal mode diagnostics | Independent package-authored composition of standard shift-invert, implicit-restart, inexact-Krylov, and biorthogonal spectral techniques | `src/trace_deflated_shiftinvert.jl` and `test/test_trace_deflated_shiftinvert.jl`. Pietracaprina *et al.*, *SciPost Phys.* **5**, 045 (2018), [DOI 10.21468/SciPostPhys.5.5.045](https://doi.org/10.21468/SciPostPhys.5.5.045), is cited as general shift-invert context, not as a source-code origin. The implementation uses the package's existing implicit-QR Arnoldi and recycled-GMRES primitives, adds PI trace deflation and original-generator certification, and copies no third-party source expression |
| Optional no-jump-resolvent-iterative/QuTiP PIQS performance harness | Interoperability and measurement against public APIs; no adapted implementation | `benchmark/comparison/no_jump_iterative_qutip/` calls documented QuTiP 5.2 PIQS and steady-state APIs plus SciPy sparse solvers from an optional Python environment. It contains package-authored coordinate matching, validation, and timing code; it neither vendors QuTiP/SciPy nor translates their source. Upstream package licenses remain authoritative for that optional environment |
