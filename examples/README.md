...
The CairoMakie examples use Makie's bundled **Computer Modern LaTeX fonts**
for regular, bold, italic, and bold-italic text, including numeric ticks and
colorbars. No system TeX installation is required. `utils/makie_support.jl`
sets the shared theme on loading the renderer, and `example_figure` also pins
the font faces locally so a later global theme reset cannot change them.
Prefer the vector PDF for publications and the PNG for slides or web pages.
Numerical controls remain independent of export resolution.

Font selection and math mode are different. `ExampleMakie.latex` preserves a
mixed text/math label: put each formula in explicit dollar delimiters, and
leave explanatory prose outside them. For example:

```julia
xlabel = ExampleMakie.latex(raw"$1/N$")
ylabel = ExampleMakie.latex(raw"$\langle J_z\rangle/N$")
title = ExampleMakie.latex("Stationary polarization, \$N=$N\$")
```

Here `N`, `J`, and `z` are mathematical variables, while the title text is
upright. Use `J_z`, not `Jz` or `J\_z`; use `\mathrm{Re}` and `\mathrm{tr}`
only for operator names, not for the entire formula. The rendering test checks
italic variables, lowered subscripts, bracket glyphs, and the supported TeX
in example label templates.

Changing a theme does not modify figures already saved on disk. Refresh named
examples, or omit the names to rerun all CairoMakie examples at their default
numerical settings (which can take substantial time):

```sh
julia --project=examples scripts/render_example_figures.jl driven_qubits boundary_time_crystal
julia --project=examples test/test_example_fonts.jl
```

The renderer accepts `--list`, honors `PI_EXAMPLE_FIGURE_DIR`, and does not
automatically replace the reviewed documentation snapshots. Old output names
from renamed scripts are not regenerated by the current scripts.
