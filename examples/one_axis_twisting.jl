...
if makie_available()
    ...
    M.Label(figure[0,1:3], "One-axis twisting  •  N=$N, χ=$chi";
            fontsize=22,font=:bold,halign=:left)
    spin_axis=M.Axis(
        figure[1,1];xlabel="χt",ylabel="⟨Jx⟩ / (N/2)",
        title="(a) Collective coherence")
    purity_axis=M.Axis(
        figure[1,2];xlabel="χt",ylabel="tr(ρ₁²)",
        title="(b) One-spin purity")
    error_axis=M.Axis(figure[1,3];xlabel="χt",ylabel="absolute error",
                      title="(c) Analytic checks")
    ...
    M.lines!(purity_axis,scaled_times,exact_purities;
             color=:black,label="exact one-spin purity")
    M.scatter!(purity_axis,scaled_times,one_body_purities;
               color=example_colors.orange,marker=:diamond,markersize=7,label="reduced PI state")
    ...
    M.lines!(error_axis,scaled_times,errors ./ (N/2);
             color=example_colors.blue,label="normalized mean spin")
    M.lines!(error_axis,scaled_times,purity_errors;
             color=example_colors.orange,linestyle=:dash,label="one-spin purity")
    M.axislegend(error_axis;position=:lt,labelsize=12)
    M.Label(figure[2,1:3], "Unitary evolution: reduced purity decreases as spins become correlated.  •  RK4: 64 steps per interval.";
