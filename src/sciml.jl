...
function dynamics_problem(x,rho0::PIState,tspan;parameters=nothing)
    x isa PIModel && x.basis!==rho0.basis &&
        throw(ArgumentError("model and initial state use incompatible PI bases"))
    x isa CompiledPIModel && x.model.basis!==rho0.basis &&
        throw(ArgumentError("compiled model and initial state use incompatible PI bases"))
    ...
    work = L isa LiouvillianPlan ? LiouvillianWorkspace(L) :
           L isa CompiledPIModel ? LiouvillianWorkspace(L.plan) :
           L isa MatrixFreeLiouvillian&&L.plan!==nothing ? LiouvillianWorkspace(L.plan) : nothing
...
Return a `PIState` from a saved solution index or from the raw solution's
continuous interpolation at `time`.
"""
state(sol::PISolution,i::Integer)=PIState(sol.basis,sol.raw.u[i])
state(sol::PISolution,t::Real)=PIState(sol.basis,sol.raw(t))
