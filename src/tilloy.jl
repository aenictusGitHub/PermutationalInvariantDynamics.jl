# No-jump resolvents and the Tilloy iterative methods ------------------------
...
[`SpecializedPIModel`](@ref), or [`TilloyPlan`](@ref). A specialization uses
...
eigendecomposition route used in the Tilloy algorithm; it rejects singular or
...
function _tilloy_add_scaled_matrix!(destination,source,scale)
...
function _tilloy_add_scaled_matrix!(destination,source::SparseMatrixCSC,scale)
...
function _tilloy_jump_scale(kernel,parameters)
    scale=_family_kernel_scale(kernel,parameters)
    scale>=0||throw(ArgumentError(
        "Tilloy CPTP and contraction methods require nonnegative jump rates; " *
        "freeze or rebuild the model with a nonnegative GKSL rate"))
    scale
end

function _tilloy_check_tolerances(atol,rtol,::Type{R}) where R
...
function _tilloy_require_hermitian_blocks(blocks,description)
    for (sector,block) in pairs(blocks)
        ishermitian(block)||throw(ArgumentError(
            ...
            "Schur sector $sector. Tilloy CPTP/contraction guarantees " *
...
_tilloy_accumulate_hamiltonian!(blocks,kernel,parameters)=blocks

function _tilloy_accumulate_hamiltonian!(blocks,
        kernel::HamiltonianPIKernel,parameters)
    scale=_family_kernel_scale(kernel,parameters)
    @inbounds for sector in eachindex(blocks)
        _tilloy_add_scaled_matrix!(blocks[sector],kernel.blocks[sector],scale)
    end
    blocks
end
...
function _tilloy_accumulate_generator!(blocks,kernel::HamiltonianPIKernel,
        parameters)
    scale=_family_kernel_scale(kernel,parameters)
    @inbounds for sector in eachindex(blocks)
        _tilloy_add_scaled_matrix!(blocks[sector],kernel.blocks[sector],
                                   -1im*scale)
    end
    0
end

function _tilloy_accumulate_loss!(blocks,qblocks,scale)
    @inbounds for sector in eachindex(blocks)
        _tilloy_add_scaled_matrix!(blocks[sector],qblocks[sector],-scale/2)
    end
    blocks
end

function _tilloy_accumulate_generator!(blocks,kernel::DissipatorPIKernel,
        parameters)
    scale=_tilloy_jump_scale(kernel,parameters)
    _tilloy_accumulate_loss!(blocks,kernel.qblocks,scale)
    1
end

function _tilloy_accumulate_generator!(blocks,kernel::Union{
...
function _tilloy_check_fused_gain_scale(scale)
    scale isa Number&&isfinite(real(scale))&&isfinite(imag(scale))&&
        iszero(imag(scale))&&real(scale)>=0||throw(ArgumentError(
        "Tilloy CPTP and contraction methods require finite nonnegative " *
        "jump rates in every fused gain channel"))
    real(scale)
end

function _tilloy_accumulate_generator!(blocks,kernel::FusedStaticPIKernel,
        parameters)
    if kernel.hamiltonian_blocks!==nothing
        @inbounds for sector in eachindex(blocks)
            _tilloy_add_scaled_matrix!(blocks[sector],
    ...
    for gain in kernel.collective_gains
        _tilloy_check_fused_gain_scale(gain.scale)
    end
...
function _tilloy_accumulate_generator!(blocks,kernel,parameters)
    throw(ArgumentError(
        "compiled kernel $(typeof(kernel)) does not expose the fixed GKSL " *
        "jump/no-jump split required by the Tilloy method; custom terms must " *
        "delegate compile_term to a supported built-in term"))
end

_tilloy_supported_kernel(::Union{
...
_tilloy_supported_kernel(::Any)=false

function _tilloy_check_supported_kernels(kernels)
    for kernel in kernels
        _tilloy_supported_kernel(kernel)||throw(ArgumentError(
            "compiled kernel $(typeof(kernel)) does not expose the fixed " *
            "GKSL jump/no-jump split required by the Tilloy method; custom " *
            "terms must delegate compile_term to a supported built-in term"))
    end
    nothing
end

function _tilloy_generator_blocks(plan::LiouvillianPlan,parameters=nothing)
    T=plan.Ttype
    ...
    for kernel in plan.kernels
        _tilloy_accumulate_hamiltonian!(
            hamiltonian_blocks,kernel,parameters)
        channels+=_tilloy_accumulate_generator!(blocks,kernel,parameters)
    end
    _tilloy_require_hermitian_blocks(
        hamiltonian_blocks,"the total prepared PI generator")
    blocks,channels
end

function _tilloy_factorization_error(backend,T)
    ArgumentError(
...
function _tilloy_check_factorization_precision(::Type{T}) where T
    R=_real_float_type(T)
    R===BigFloat&&throw(ArgumentError(
        "Tilloy no-jump plans do not currently support BigFloat: Julia's " *
...
function _tilloy_schur_factor(block)
    decomposition=try
        schur(block)
    catch error
        error isa MethodError||rethrow()
        throw(_tilloy_factorization_error(:schur,eltype(block)))
    end
...
function _tilloy_eigen_factor(block,condition_limit)
    decomposition=try
        eigen(block)
    catch error
        error isa MethodError||rethrow()
        throw(_tilloy_factorization_error(:eigen,eltype(block)))
    end
...
function _tilloy_plan_storage_estimate(basis,::Type{T}) where T
...
function _tilloy_workspace_estimate(plan::NoJumpResolventPlan)
...
function _prepare_nojump_resolvent_plan(plan::LiouvillianPlan,parameters;
    ...
    require_autonomous&& !isautonomous(plan)&&throw(ArgumentError(
        "the Tilloy no-jump split requires an autonomous fixed model; freeze " *
        "time-dependent rates and operators first"))
    plan.kernels===nothing&&throw(ArgumentError(
        "the Tilloy no-jump split requires prepared built-in kernels"))
    _tilloy_check_supported_kernels(plan.kernels)
    T=plan.Ttype
    R=_real_float_type(T)
    ...
    _tilloy_check_factorization_precision(T)
    ...
    estimate=BigInt(Base.summarysize(plan))+
        _tilloy_plan_storage_estimate(plan.basis,T)
    _require_performance_budget("Tilloy no-jump factorization",estimate,
        memory_budget;guidance="Reduce the retained basis or increase the budget.")
    blocks,jump_channels=_tilloy_generator_blocks(plan,parameters)
    factors=backend===:schur ? map(_tilloy_schur_factor,blocks) :
        map(block->_tilloy_eigen_factor(block,limit),blocks)
...
function _tilloy_reject_evaluation_arguments(source,time,parameters)
...
function _tilloy_instantaneous_model(model::PIModel,time,parameters)
    ...
    time===nothing&&throw(ArgumentError(
        ...
        "instantaneous Tilloy preparation"))
    time isa Real&&isfinite(time)||throw(ArgumentError(
        "instantaneous Tilloy preparation time must be finite and real"))
...
function _tilloy_with_metadata(plan::NoJumpResolventPlan,metadata)
...
function NoJumpResolventPlan(plan::LiouvillianPlan;time=nothing,
    ...
    _tilloy_reject_evaluation_arguments(plan,time,parameters)
...
function NoJumpResolventPlan(model::PIModel;
    ...
    _require_model_preparation_budget(model,budget;
        operation="Tilloy model preparation",coefficient_cache)
    frozen,metadata=_tilloy_instantaneous_model(model,time,parameters)
    plan=NoJumpResolventPlan(
        LiouvillianPlan(frozen;coefficient_cache);kwargs...)
    _tilloy_with_metadata(plan,metadata)
end

function NoJumpResolventPlan(compiled::CompiledPIModel;time=nothing,
        parameters=nothing,kwargs...)
    _tilloy_reject_evaluation_arguments(compiled,time,parameters)
    NoJumpResolventPlan(compiled.plan;kwargs...)
end

function NoJumpResolventPlan(specialized::SpecializedPIModel;time=nothing,
        parameters=nothing,kwargs...)
    _tilloy_reject_evaluation_arguments(specialized,time,parameters)
...
function NoJumpResolventWorkspace(plan::NoJumpResolventPlan;
        memory_budget=_DEFAULT_HIGHLEVEL_MEMORY_BUDGET)
    estimate=BigInt(Base.summarysize(plan))+
        _tilloy_workspace_estimate(plan)
...
function _tilloy_shift(plan::NoJumpResolventPlan,shift)
    ...
    iszero(converted)&&!plan.metadata.strictly_stable&&throw(ArgumentError(
        ...
        "This is the dark-state branch of the Tilloy theorem; use a positive " *
...
@inline function _tilloy_checked_resolvent_division(
...
@inline function _tilloy_checked_resolvent_accumulate(
...
function _tilloy_check_resolvent_output(destination,backend::Symbol)
...
function _solve_nojump_sector!(transformed,triangular,shift)
    ...
    @inbounds for column in n:-1:1,row in n:-1:1
        ...
        for index in row+1:n
            value=_tilloy_checked_resolvent_accumulate(value,
        ...
        transformed[row,column]=_tilloy_checked_resolvent_division(
...
function _apply_nojump_sector!(destination,source,
    ...
    _tilloy_check_resolvent_output(destination,:Schur)
end
...
function _apply_nojump_sector!(destination,source,
    ...
    @inbounds for column in axes(right,2),row in axes(right,1)
        ...
        right[row,column]=_tilloy_checked_resolvent_division(
            right[row,column],denominator,:eigen)
    end
    mul!(left,factor.vectors,right)
    mul!(destination,left,adjoint(factor.vectors))
    _tilloy_check_resolvent_output(destination,:eigen)
end
...
function no_jump_resolvent!(destination::AbstractVector,
    ...
    lambda=_tilloy_shift(plan,shift)
    for sector in eachindex(plan.basis.sectors)
...
"""
    TilloyPlan(source; backend=:schur, memory_budget=512*1024^2, ...)

Prepare the exact PI jump/no-jump splitting used by the Tilloy fixed-point,
...
`TilloyPlan(model; time=t, parameters=p)`. This is the stationary/spectral
problem for that frozen generator, not the long-time state of driven dynamics.
"""
struct TilloyPlan{B,L,N,V,D,T,M}
    basis::B
    liouvillian::L
    no_jump::N
    tracevec::V
...
size(plan::TilloyPlan)=size(plan.no_jump)
size(plan::TilloyPlan,index::Integer)=size(plan.no_jump,index)
eltype(plan::TilloyPlan)=plan.Ttype

function _tilloy_source_adjoint!(destination,source::LiouvillianPlan,
        tracevec,workspace)
    apply_adjoint!(destination,source,tracevec,workspace)
end

function _tilloy_source_adjoint!(destination,source::SpecializedPIModel,
        tracevec,workspace)
...
function _tilloy_trace_preservation_certificate(source,
    ...
    atol,rtol=_tilloy_check_tolerances(trace_atol,requested_rtol,R)
    ...
    isfinite(scale)||throw(ArgumentError(
        "Tilloy trace-preservation certification scale is nonfinite; " *
        "use a wider prepared scalar type"))
    tolerance=atol+rtol*scale
    isfinite(tolerance)||throw(ArgumentError(
        "Tilloy trace-preservation tolerance overflowed; use a wider " *
    ...
    _require_performance_budget(
        "Tilloy trace-preservation certification",estimate,memory_budget;
...
function TilloyPlan(plan::LiouvillianPlan;trace_atol::Real=0,
        trace_rtol=nothing,
        memory_budget=_DEFAULT_HIGHLEVEL_MEMORY_BUDGET,time=nothing,
        parameters=nothing,kwargs...)
    _tilloy_reject_evaluation_arguments(plan,time,parameters)
    no_jump=NoJumpResolventPlan(plan;memory_budget,kwargs...)
    trace_certificate=_tilloy_trace_preservation_certificate(
    ...
    TilloyPlan(plan.basis,plan,no_jump,plan.tracevec,deflation,
               plan.Ttype,metadata)
end

function _tilloy_with_metadata(plan::TilloyPlan,metadata)
    metadata===nothing&&return plan
    no_jump=_tilloy_with_metadata(plan.no_jump,metadata)
    TilloyPlan(plan.basis,plan.liouvillian,no_jump,plan.tracevec,
        plan.deflation_vector,plan.Ttype,merge(plan.metadata,metadata))
end

function TilloyPlan(model::PIModel;coefficient_cache=nothing,time=nothing,
        parameters=nothing,kwargs...)
    budget=get(kwargs,:memory_budget,_DEFAULT_HIGHLEVEL_MEMORY_BUDGET)
    _require_model_preparation_budget(model,budget;
        operation="Tilloy model preparation",coefficient_cache)
    frozen,metadata=_tilloy_instantaneous_model(model,time,parameters)
    plan=TilloyPlan(LiouvillianPlan(frozen;coefficient_cache);kwargs...)
    _tilloy_with_metadata(plan,metadata)
end

function TilloyPlan(compiled::CompiledPIModel;time=nothing,
        parameters=nothing,kwargs...)
    _tilloy_reject_evaluation_arguments(compiled,time,parameters)
    TilloyPlan(compiled.plan;kwargs...)
end

function TilloyPlan(specialized::SpecializedPIModel;trace_atol::Real=0,
        trace_rtol=nothing,
        memory_budget=_DEFAULT_HIGHLEVEL_MEMORY_BUDGET,time=nothing,
        parameters=nothing,kwargs...)
    _tilloy_reject_evaluation_arguments(specialized,time,parameters)
    ...
    metadata=merge(no_jump.metadata,trace_certificate,(
        ...
        unique_steady_state=:assumed_not_certified,
        unsupported_sources=(:composite,:global_pseudomode,:heom,:hops)))
    TilloyPlan(specialized.plan.basis,specialized,no_jump,
        specialized.plan.tracevec,deflation,specialized.plan.Ttype,metadata)
end

isautonomous(::TilloyPlan)=true

function NoJumpResolventPlan(plan::TilloyPlan;kwargs...)
    isempty(kwargs)||throw(ArgumentError(
        "a TilloyPlan already owns a prepared no-jump resolvent; constructor " *
        "keywords cannot rebuild it implicitly. Construct a new TilloyPlan " *
        "with the requested options instead."))
    plan.no_jump
end

_tilloy_underlying_plan(plan::TilloyPlan)=
    plan.liouvillian isa SpecializedPIModel ?
        plan.liouvillian.plan : plan.liouvillian

function _apply_tilloy_liouvillian!(destination,plan::TilloyPlan,source,
...
_tilloy_gain_parameters(plan::TilloyPlan)=
    plan.liouvillian isa SpecializedPIModel ? plan.liouvillian.rates : nothing

function _apply_tilloy_gain_kernel!(y,x,::HamiltonianPIKernel,prepared,b,
        parameters,work)
    nothing
end

function _apply_tilloy_gain_kernel!(y,x,kernel::DissipatorPIKernel,
        prepared,b,parameters,work)
    scale=convert(eltype(work[1][1]),
        _tilloy_jump_scale(kernel,parameters))
...
function _apply_tilloy_gain_kernel!(y,x,kernel::LocalJumpPIKernel,
        prepared,b,parameters,work)
    scale=convert(eltype(work[1][1]),
        _tilloy_jump_scale(kernel,parameters))
...
function _apply_tilloy_gain_kernel!(y,x,
...
function _apply_tilloy_gain_kernel!(y,x,kernel::FusedStaticPIKernel,
        prepared::StaticFactorizedGainKernelWorkspace,b,parameters,work)
    for gain in kernel.collective_gains
        scale=convert(eltype(work[1][1]),
            _tilloy_check_fused_gain_scale(gain.scale))
...
function _apply_tilloy_gain_kernel!(y,x,kernel,prepared,b,parameters,work)
    throw(ArgumentError(
        "compiled kernel $(typeof(kernel)) does not expose the fixed gain " *
        "map required by the Tilloy method"))
end

@inline _apply_tilloy_gain_kernels!(y,x,::Tuple{},::Tuple{},b,parameters,
    work)=nothing
@inline function _apply_tilloy_gain_kernels!(y,x,
        kernels::Tuple{K,Vararg{Any}},
        prepared::Tuple{W,Vararg{Any}},b,parameters,work) where {K,W}
    _apply_tilloy_gain_kernel!(
        y,x,first(kernels),first(prepared),b,parameters,work)
    _apply_tilloy_gain_kernels!(y,x,Base.tail(kernels),
        Base.tail(prepared),b,parameters,work)
end

function _apply_tilloy_gain!(destination,plan::TilloyPlan,source,
        workspace::LiouvillianWorkspace)
    underlying=_tilloy_underlying_plan(plan)
    underlying.kernels===nothing&&throw(ArgumentError(
        "the Tilloy gain map requires prepared physical kernels"))
    Base.mightalias(destination,source)&&throw(ArgumentError(
        "Tilloy gain-map source and destination must not alias"))
    length(destination)==length(plan.basis)&&
        length(source)==length(plan.basis)||throw(DimensionMismatch(
            "Tilloy gain-map vectors have the wrong length"))
    _check_liouvillian_workspace(workspace,underlying)
    _check_liouvillian_apply_types(destination,source,underlying)
    parameters=_tilloy_gain_parameters(plan)
    ...
    _apply_tilloy_gain_kernels!(destination,source,underlying.kernels,
        workspace.kernel_workspaces,plan.basis,parameters,workspace.blocks)
    destination
end

"""
    TilloyWorkspace(plan; krylovdim=30, recycle_dim=8,
                     memory_budget=512*1024^2)

Task-owned no-jump, Liouvillian, and recycled-GMRES scratch for Tilloy linear
...
"""
mutable struct TilloyWorkspace{P,N,L,G,T,R}
    plan::P
    no_jump::N
    liouvillian::L
    gmres::G
...
function _tilloy_workspace_estimate(plan::TilloyPlan,krylovdim,recycle_dim)
    ...
    _tilloy_workspace_estimate(plan.no_jump)+
        _performance_liouvillian_workspace_bytes(
            _tilloy_underlying_plan(plan))+
        _performance_gmres_bytes(n,T,krylovdim;recycle_dim)+
        _performance_entries_bytes(7BigInt(n),T)+projected_transient
end

function TilloyWorkspace(plan::TilloyPlan;krylovdim::Integer=30,
    ...
    estimate=BigInt(Base.summarysize(plan))+
        _tilloy_workspace_estimate(plan,krylovdim,recycle_dim)
    _require_performance_budget("Tilloy iterative workspace",estimate,
        memory_budget;guidance=
    ...
    TilloyWorkspace(plan,NoJumpResolventWorkspace(plan.no_jump;
            memory_budget=Inf),LiouvillianWorkspace(
                _tilloy_underlying_plan(plan)),
        RecycledGMRESWorkspace(T,n,krylovdim,recycle_dim),vectors...,
        R(NaN),R(NaN),one(T),false,false,BigInt(estimate))
end

function _check_tilloy_workspace(work::TilloyWorkspace,plan::TilloyPlan)
    work.plan===plan||throw(ArgumentError(
        "Tilloy workspace belongs to a different prepared plan"))
    _check_nojump_workspace(work.no_jump,plan.no_jump)
    _check_liouvillian_workspace(
        work.liouvillian,_tilloy_underlying_plan(plan))
    work
end

function _tilloy_workspace_vectors(work::TilloyWorkspace)
...
function _tilloy_external_vector_bytes(work::TilloyWorkspace,array)
    any(owner->Base.mightalias(array,owner),
        _tilloy_workspace_vectors(work))&&return big(0)
    _performance_entries_bytes(BigInt(length(array)),eltype(array))
end

function _tilloy_state_validation_bytes(plan::TilloyPlan)
...
function _tilloy_stationary_output_bytes(plan::TilloyPlan)
    _performance_entries_bytes(BigInt(length(plan.basis)),plan.Ttype)+
        _tilloy_state_validation_bytes(plan)
end

function _tilloy_outer_arnoldi_bytes(plan::TilloyPlan,nev,krylovdim,
...
function _tilloy_solver_scalar(plan::TilloyPlan,value,name)
...
function _prepare_tilloy_preconditioner!(work::TilloyWorkspace,
        plan::TilloyPlan,shift,deflation;singularity_atol::Real=0,
        singularity_rtol=nothing)
    lambda=_tilloy_solver_scalar(plan,shift,"Tilloy shift")
    delta=_tilloy_solver_scalar(plan,deflation,"Tilloy deflation")
    if !(work.cache_valid&&lambda==work.cached_shift&&
        ...
        if iszero(delta)
            ...
            no_jump_resolvent!(work.identity_resolvent,plan.no_jump,
    ...
    requested_rtol=singularity_rtol===nothing ? sqrt(eps(R)) :
        singularity_rtol
    atol,rtol=_tilloy_check_tolerances(
...
function _apply_tilloy_preconditioner!(destination,plan::TilloyPlan,source,
        work::TilloyWorkspace,shift,deflation)
    no_jump_resolvent!(destination,plan.no_jump,source,shift,work.no_jump)
    if !iszero(deflation)
        coefficient=deflation*dot(plan.tracevec,destination)/work.denominator
...
function _apply_tilloy_shifted_deflated!(destination,plan::TilloyPlan,
        source,work::TilloyWorkspace,shift,deflation)
    _apply_tilloy_liouvillian!(work.image,plan,source,work.liouvillian)
...
function _tilloy_physical_maximum(basis,data)
...
function _tilloy_reset_linear_workspace!(work::TilloyWorkspace)
...
function _check_tilloy_linear_arrays(destination,right_hand_side,
        plan::TilloyPlan)
    ...
    source_type<:Number&&isconcretetype(source_type)||throw(ArgumentError(
        "Tilloy right-hand sides must expose a concrete numeric eltype"))
    destination_type<:Number&&isconcretetype(destination_type)||
        throw(ArgumentError(
            "Tilloy destinations must expose a concrete numeric eltype"))
    promote_type(plan.Ttype,source_type)===plan.Ttype||throw(ArgumentError(
        "Tilloy right-hand-side scalar type $source_type is wider than " *
        "prepared precision $(plan.Ttype)"))
    promote_type(destination_type,plan.Ttype)===destination_type||
        throw(ArgumentError(
            "Tilloy destination scalar type $destination_type cannot " *
            "represent prepared precision $(plan.Ttype)"))
    nothing
end

"""
    tilloy_resolvent!(destination, plan, right_hand_side, workspace;
...
shift-invert solves.
"""
function tilloy_resolvent!(destination::AbstractVector,plan::TilloyPlan,
        right_hand_side::AbstractVector,work::TilloyWorkspace;
        shift::Real,deflation::Real=0,reuse::Bool=false,
        ...
        memory_budget=_DEFAULT_HIGHLEVEL_MEMORY_BUDGET)
    _check_tilloy_workspace(work,plan)
    length(destination)==length(plan.basis)&&
        length(right_hand_side)==length(plan.basis)||throw(DimensionMismatch(
            "Tilloy resolvent vectors have the wrong length"))
    Base.mightalias(destination,right_hand_side)&&throw(ArgumentError(
        "Tilloy resolvent source and destination must not alias"))
    _check_tilloy_linear_arrays(destination,right_hand_side,plan)
    !(maxiter isa Bool)&&maxiter>0||throw(ArgumentError(
        "maxiter must be a positive integer"))
    R=_real_float_type(plan.Ttype)
    atolT,rtolT=_tilloy_check_tolerances(atol,rtol,R)
    estimate=work.accounted_peak_bytes+
        _tilloy_external_vector_bytes(work,destination)+
        _tilloy_external_vector_bytes(work,right_hand_side)
    _require_performance_budget("Tilloy resolvent solve",
        estimate,
        memory_budget;guidance=
            "Use a smaller prepared workspace or increase the budget.")
    lambda,delta=_prepare_tilloy_preconditioner!(work,plan,shift,deflation;
        singularity_atol,singularity_rtol)
    copyto!(work.rhs,right_hand_side)
    reuse&&work.warm_start_valid||_tilloy_reset_linear_workspace!(work)
    function right_preconditioned_action!(output,input)
        _apply_tilloy_preconditioner!(work.preconditioned,plan,input,work,
                                      lambda,delta)
        _apply_tilloy_shifted_deflated!(output,plan,work.preconditioned,
                                        work,lambda,delta)
    end
    ...
    _apply_tilloy_preconditioner!(work.physical_solution,plan,
        work.transformed_solution,work,lambda,delta)
    copyto!(destination,work.physical_solution)
    _apply_tilloy_shifted_deflated!(work.residual,plan,destination,work,
                                    lambda,delta)
    @. work.residual=work.residual-work.rhs
    residual=norm(work.residual)
    residual_inf=norm(work.residual,Inf)
    physical_residual_inf=_tilloy_physical_maximum(plan.basis,work.residual)
    ...
    if require_convergence&&!converged
        throw(ArgumentError(
            "Tilloy right-preconditioned GMRES did not converge in " *
    ...
    (solution=destination,converged,residual,residual_inf,
     ...
     right_preconditioned=true,workspace_reused=reuse)
end

"""
    tilloy_resolvent(plan, right_hand_side; shift, workspace=nothing, ...)

Allocating wrapper for [`tilloy_resolvent!`](@ref). The returned named tuple
contains the solution vector and true residual diagnostics.
"""
function tilloy_resolvent(plan::TilloyPlan,
        right_hand_side::AbstractVector;shift::Real,workspace=nothing,
        krylovdim::Integer=30,recycle_dim::Integer=8,
        memory_budget=_DEFAULT_HIGHLEVEL_MEMORY_BUDGET,kwargs...)
    work=workspace===nothing ? TilloyWorkspace(plan;krylovdim,recycle_dim,
        memory_budget) : _check_tilloy_workspace(workspace,plan)
    output_bytes=_performance_entries_bytes(
        BigInt(length(plan.basis)),plan.Ttype)
    estimate=work.accounted_peak_bytes+
        _tilloy_external_vector_bytes(work,right_hand_side)+output_bytes
    _require_performance_budget("allocating Tilloy resolvent",estimate,
        memory_budget;guidance=
            "Use tilloy_resolvent! with caller-owned storage or increase " *
            "the budget.")
    destination=zeros(plan.Ttype,length(plan.basis))
    tilloy_resolvent!(destination,plan,right_hand_side,work;
        shift,memory_budget,kwargs...)
end

struct _TilloySizedOperator{F,T,W}
    n::Int
    action!::F
    Ttype::Type{T}
    action_workspace::W
end
size(operator::_TilloySizedOperator)=(operator.n,operator.n)
size(operator::_TilloySizedOperator,index::Integer)=
    index in (1,2) ? operator.n : 1
eltype(operator::_TilloySizedOperator)=operator.Ttype
function mul!(destination::AbstractVector,operator::_TilloySizedOperator,
              source::AbstractVector)
    operator.action!(destination,source)
    destination
end
# The internal sized adapter applies into its already-retained, explicitly
# task-owned Tilloy workspace. It has no additional per-application vector
# allowance. High-level Tilloy consumers compose that retained workspace with
# their outer Krylov storage explicitly before allocating it.
_performance_source_action_bytes(::_TilloySizedOperator,::Type)=big(0)

function _tilloy_stationary_diagnostics(plan,state,work,atol,rtol)
    _apply_tilloy_liouvillian!(
...
function _require_tilloy_stationary_result(diagnostics,atol,rtol)
    ...
    diagnostics.physical_residual_inf<=tolerance||throw(ArgumentError(
        "Tilloy stationary candidate fails the true Liouvillian residual " *
        "check: physical max residual=$(diagnostics.physical_residual_inf), " *
        "tolerance=$tolerance"))
    diagnostics.state_diagnostics.valid||throw(ArgumentError(
        "Tilloy stationary candidate is not a valid density state within " *
        "the requested tolerances; diagnostics=$(diagnostics.state_diagnostics)"))
    nothing
end

function _tilloy_stationary_linear_tolerances(plan::TilloyPlan,deflation,
        atol,rtol)
    R=_real_float_type(plan.Ttype)
    state_atol,state_rtol=_tilloy_check_tolerances(atol,rtol,R)
    ...
    trace_norm>zero(R)&&isfinite(trace_norm)||throw(ArgumentError(
        "Tilloy trace functional has zero or nonfinite norm"))
    rhs_norm>zero(R)&&isfinite(rhs_norm)||throw(ArgumentError(
        "Tilloy trace-deflated stationary right-hand side has zero or " *
        "nonfinite norm"))
    ...
    isfinite(target)||throw(ArgumentError(
        "Tilloy trace-aware stationary residual target is nonfinite"))
    iszero(target)&&!iszero(deflation)&&!iszero(state_trace_tolerance)&&
        throw(ArgumentError(
            "Tilloy trace-aware stationary residual target underflowed in " *
...
"""
    tilloy_steady_state(source; method=:gmres, ...)

Compute a PI stationary state with the Tilloy jump/no-jump split. `source` may
be a [`PIModel`](@ref), [`CompiledPIModel`](@ref),
[`SpecializedPIModel`](@ref), or prepared [`TilloyPlan`](@ref).
...
"""
function tilloy_steady_state(plan::TilloyPlan;method::Symbol=:gmres,
    ...
    method in (:gmres,:fixed_point)||throw(ArgumentError(
        "Tilloy steady-state method must be :gmres or :fixed_point"))
    R=_real_float_type(plan.Ttype)
    atolT,rtolT=_tilloy_check_tolerances(atol,rtol,R)
    ...
    work=workspace===nothing ?
        (method===:gmres ? TilloyWorkspace(plan;krylovdim,recycle_dim,
            memory_budget) : TilloyWorkspace(plan;krylovdim=1,
            recycle_dim=0,memory_budget)) :
        _check_tilloy_workspace(workspace,plan)
    if method===:gmres
        _require_performance_budget("Tilloy stationary-state solve",
            work.accounted_peak_bytes+
                _tilloy_stationary_output_bytes(plan),
            memory_budget;guidance=
        ...
        trace_control=_tilloy_stationary_linear_tolerances(
            plan,delta,atolT,rtolT)
        linear=tilloy_resolvent!(work.physical_solution,plan,work.rhs,work;
        ...
        stationary=_tilloy_stationary_diagnostics(
            plan,state,work,atolT,rtolT)
        _require_tilloy_stationary_result(stationary,atolT,rtolT)
        ...
        info=(state,method=:tilloy_gmres,converged=true,
    ...
    fixed_point_peak=work.accounted_peak_bytes+
        _tilloy_outer_arnoldi_bytes(
            plan,1,krylovdim,maxrestarts;vectors=true)+
        _performance_entries_bytes(BigInt(length(plan.basis)),plan.Ttype)+
        _tilloy_stationary_output_bytes(plan)
    _require_performance_budget("Tilloy fixed-point stationary solve",
    ...
    operator=_TilloySizedOperator(length(plan.basis),fixed_point_action!,
                                  plan.Ttype,work)
    ...
    isfinite(real(normalization))&&isfinite(imag(normalization))&&
        !iszero(normalization)||throw(ArgumentError(
            "Tilloy fixed-point Ritz vector reconstructs a state with zero " *
            "or nonfinite trace"))
    ...
    stationary=_tilloy_stationary_diagnostics(
        plan,state,work,atolT,rtolT)
    _require_tilloy_stationary_result(stationary,atolT,rtolT)
    info=(state,method=:tilloy_fixed_point,converged=true,
...
function _tilloy_plan_from_source(source;backend,memory_budget,
        coefficient_cache,time,parameters)
    if source isa PIModel
        return TilloyPlan(source;backend,memory_budget,coefficient_cache,
                          time,parameters)
    end
    ...
    _tilloy_reject_evaluation_arguments(source,time,parameters)
    TilloyPlan(source;backend,memory_budget)
end

function tilloy_steady_state(
        ...
        coefficient_cache=nothing,time=nothing,parameters=nothing,kwargs...)
    plan=_tilloy_plan_from_source(source;backend,memory_budget,
        coefficient_cache,time,parameters)
    tilloy_steady_state(plan;memory_budget,kwargs...)
end

"""
    tilloy_liouvillian_spectrum(source; nev=3, shift=0, deflation=1,
                                candidate_oversampling=nothing, ...)

Compute selected nonzero low-lying Liouvillian modes with thick-restarted
shift-invert Arnoldi. Every inverse action is a Tilloy right-preconditioned,
...
"""
function tilloy_liouvillian_spectrum(plan::TilloyPlan;nev::Integer=3,
    ...
    inner_atolT,inner_rtolT=
        _tilloy_check_tolerances(inner_atol,inner_rtol,R)
    lambda=_tilloy_solver_scalar(plan,shift,"Tilloy spectral shift")
    delta=_tilloy_solver_scalar(plan,deflation,"Tilloy deflation")
    iszero(delta)&&throw(ArgumentError(
    ...
    operator=_TilloySizedOperator(n,inverse_action!,plan.Ttype,work)
    ...
    spectrum_peak=work.accounted_peak_bytes+
        ...
        selection_bytes
    _require_performance_budget("Tilloy shift-invert spectrum",
    ...
    for column in axes(outer.vectors,2)
        ...
        residual=norm(work.residual)
        physical=_tilloy_physical_maximum(plan.basis,work.residual)
        trace_error=abs(dot(plan.tracevec,vector))
        scale=max(_tilloy_physical_maximum(plan.basis,work.image),
                  abs(value)*_tilloy_physical_maximum(plan.basis,vector),
                  floatmin(R))
        tolerance=atolT+rtolT*scale
    ...
    if length(order)<nev
        message="Tilloy shift-invert produced only $(length(order)) of $nev " *
    ...
    result=(values=kept_values,residuals=kept_residuals,
        ...
        converged,method=:tilloy_shift_invert,shift=lambda,
...
function tilloy_liouvillian_spectrum(
        source::Union{PIModel,CompiledPIModel,SpecializedPIModel};
    ...
    plan=_tilloy_plan_from_source(source;backend,memory_budget,
        coefficient_cache,time,parameters)
    tilloy_liouvillian_spectrum(plan;memory_budget,kwargs...)
end

"""
    tilloy_implicit_euler_step!(destination, plan, source, dt, workspace; ...)

Advance one autonomous master-equation step with
``destination=(I-dt*L)^(-1)*source``. The solve is written at the positive
shift `1/dt`, where the Tilloy no-jump error map is a strict trace-norm
...
"""
function tilloy_implicit_euler_step!(destination::PIState,plan::TilloyPlan,
        source::PIState,dt::Real,work::TilloyWorkspace;
        reuse::Bool=true,kwargs...)
    destination.basis===plan.basis&&source.basis===plan.basis||
        throw(ArgumentError(
            "implicit-Euler states and Tilloy plan use different PI bases"))
    _check_tilloy_workspace(work,plan)
    _check_tilloy_linear_arrays(destination.data,source.data,plan)
    ...
    shift=_tilloy_solver_scalar(plan,inv(step),"implicit-Euler shift")
    @inbounds @simd for index in eachindex(work.rhs,source.data)
        work.rhs[index]=shift*source.data[index]
    end
    info=tilloy_resolvent!(destination.data,plan,work.rhs,work;
        shift,deflation=zero(R),reuse,kwargs...)
    merge(info,(step=step,method=:tilloy_implicit_euler,))
end

"""
    tilloy_implicit_euler(source, initial_state, times; ...)

Evolve an autonomous PI model on a strictly increasing saved-time grid with
first-order implicit Euler and the Tilloy positive-shift resolvent. Returns a
...
"""
function tilloy_implicit_euler(plan::TilloyPlan,initial_state::PIState,times;
        workspace=nothing,krylovdim::Integer=30,recycle_dim::Integer=8,
    ...
    initial_state.basis===plan.basis||throw(ArgumentError(
        "initial state and Tilloy plan use different PI bases"))
    ...
    estimate=workspace_peak+output_bytes+
        ...
        _tilloy_state_validation_bytes(plan)
    _require_performance_budget("Tilloy implicit-Euler saved history",
    ...
    work=prepared_workspace===nothing ? TilloyWorkspace(
    ...
    for index in 2:length(saved)
        info=tilloy_implicit_euler_step!(next,plan,current,
    ...
    (times=saved,states,diagnostics,converged=all(info->info.converged,
        diagnostics),method=:tilloy_implicit_euler,order=1,
        backend=plan.no_jump.metadata.backend,
        generator_mode=plan.metadata.generator_mode,
        unique_steady_state=:not_applicable)
end

function tilloy_implicit_euler(
        source::Union{PIModel,CompiledPIModel,SpecializedPIModel},
    ...
    plan=_tilloy_plan_from_source(source;backend,memory_budget,
        coefficient_cache,time,parameters)
    tilloy_implicit_euler(plan,initial_state,times;memory_budget,kwargs...)
end
