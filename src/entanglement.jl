...
function _su2_cgc_small(j1::Int,m1::Int,j2::Int,m2::Int,j::Int,m::Int)
    a=(j1+j2-j)÷2; bb=(j1-j2+j)÷2; c=(-j1+j2+j)÷2
    pref=sqrt(Float64((big(j+1)*_fact(a)*_fact(bb)*_fact(c))//_fact((j1+j2+j)÷2+1)))
    nums=_fact((j1+m1)÷2)*_fact((j1-m1)÷2)*_fact((j2+m2)÷2)*
         _fact((j2-m2)÷2)*_fact((j+m)÷2)*_fact((j-m)÷2)
    pref*=sqrt(Float64(nums))
    lo=max(0,-((j-j2+m1)÷2),-((j-j1-m2)÷2))
    hi=min(a,(j1-m1)÷2,(j2+m2)÷2)
    s=0.0
    for z in lo:hi
        den=_fact(z)*_fact(a-z)*_fact((j1-m1)÷2-z)*_fact((j2+m2)÷2-z)*
            _fact((j-j2+m1)÷2+z)*_fact((j-j1-m2)÷2+z)
...
function _su2_cgc_exact(j1::Int,m1::Int,j2::Int,m2::Int,j::Int,m::Int)
    a=(j1+j2-j)÷2; bb=(j1-j2+j)÷2; c=(-j1+j2+j)÷2
    A=(j1-m1)÷2; B=(j2+m2)÷2
    ...
    prefactor_squared=
        (big(j+1)*_fact(a)*_fact(bb)*_fact(c))//
        _fact((j1+j2+j)÷2+1)
    prefactor_squared*=
        _fact((j1+m1)÷2)*_fact(A)*_fact((j2+m2)÷2)*
        _fact((j2-m2)÷2)*_fact((j+m)÷2)*_fact((j-m)÷2)

    first_denominator=
        _fact(lo)*_fact(a-lo)*_fact(A-lo)*_fact(B-lo)*
        _fact(C+lo)*_fact(D+lo)
...
function _su2_cgc(j1::Int,m1::Int,j2::Int,m2::Int,j::Int,m::Int)
    ...
    max(j1,j2,j)<=32 && return _su2_cgc_small(j1,m1,j2,m2,j,m)
    _su2_cgc_exact(j1,m1,j2,m2,j,m)
end
