...
- `NoJumpResolventPlan` and `NoJumpIterativePlan` prepare the autonomous GKSL
  ...
  `NoJumpIterativeWorkspace` are task-owned. The zero-shift methods require a strictly
  stable no-jump generator and assume, but do not certify, a unique stationary
  state. `TraceDeflatedShiftInvertPlan` and its task-owned
  `TraceDeflatedShiftInvertWorkspace` add complex interior shifts, adaptive
  inexact GMRES actions, true implicit-QR IRAM, and optional paired left/right
  mode conditioning diagnostics. Every accepted mode is checked against the
  original undeflated Liouvillian. Read the [no-jump-resolvent iterative-solver
  guide](no_jump_iterative_solvers.md) before using these specialized solvers.
  These research interfaces are exported by the parent module but are
  deliberately not yet part of the stable `Workflow` namespace.
